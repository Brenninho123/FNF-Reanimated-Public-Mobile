...
class CharacterEditorState extends MusicBeatState
{
	...
	var customNoteAssets:FlxUIInputText;
	var customNotePath:FlxUIInputText;
	var customNoteSplashes:FlxUIInputText;
	var customStrumCovers:FlxUIInputText;
	...
	function addCharacterUI()
	{
		...
		customNoteAssets = new FlxUIInputText(healthColorStepperR.x, healthColorStepperR.y + 40, 85, character.noteAssets, 8);
		customNotePath = new FlxUIInputText(customNoteAssets.x + customNoteAssets.width + 5, customNoteAssets.y, 85, character.notePath, 8);
		customNoteSplashes = new FlxUIInputText(customNoteAssets.x, customNoteAssets.y + 40, 85, character.noteSplashes, 8);
		customStrumCovers = new FlxUIInputText(customNotePath.x, customNoteSplashes.y, 85, character.strumCovers, 8);
		var setNoteVals:FlxButton = new FlxButton(customNotePath.x + customNotePath.width + 5, customNotePath.y, "Set Note Values", function()
		{
			character.noteAssets = customNoteAssets.text;
			character.notePath = customNotePath.text;
			character.noteSplashes = customNoteSplashes.text;
			character.strumCovers = customStrumCovers.text;
			trace("New char notes: " + character.noteAssets);
			trace("New notes path: " + character.notePath);
			trace("New note splashes: " + character.noteSplashes);
			trace("New strum covers: " + character.strumCovers);
		});
		//setNoteVals.scale.x = 1.5;
		setNoteVals.label.fieldWidth = setNoteVals.width;
		...
		// Torch - This is for custom Note Styles per character
		tab_group.add(new FlxText(customNoteAssets.x, customNoteAssets.y - 18, 0, 'Note Assets:'));
		tab_group.add(new FlxText(customNotePath.x, customNotePath.y - 18, 0, 'Notes Path:'));
		tab_group.add(new FlxText(customNoteSplashes.x, customNoteSplashes.y - 18, 0, 'Notes Splashes:'));
		tab_group.add(new FlxText(customStrumCovers.x, customStrumCovers.y - 18, 0, 'Strum Covers:'));
		...
		// Torch - This is for custom Note Styles per character
		tab_group.add(customNoteAssets);
		tab_group.add(customNotePath);
		tab_group.add(customNoteSplashes);
		tab_group.add(customStrumCovers);
	...
	function reloadCharacterOptions() {
		...
		// Torch - This is for custom Note Styles per character
		customNoteAssets.text = character.noteAssets;
		customNotePath.text = character.notePath;
		customNoteSplashes.text = character.noteSplashes;
		customStrumCovers.text = character.strumCovers;
	...
	override function update(elapsed:Float)
	{
		...
		if(animationInputText.hasFocus || animationNameInputText.hasFocus || animationIndicesInputText.hasFocus || imageInputText.hasFocus || healthIconInputText.hasFocus || vocalsInputText.hasFocus || customNoteAssets.hasFocus || customNotePath.hasFocus || customNoteSplashes.hasFocus || customStrumCovers.hasFocus)
		{
	...
	function saveCharacter() {
		...
		var json:Dynamic = {
			...
			"noteAssets": character.noteAssets,
			"notePath": character.notePath,
			"noteSplashes" : character.noteSplashes,
			"strumCovers" : character.strumCovers
		};
