...
class WeekEditorState extends MusicBeatState
{
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Paths.modsImages('storyMenu/weekImages/' + assetName)) || #end
			Assets.exists(Paths.getPath('images/storyMenu/weekImages/' + assetName + '.png', IMAGE), IMAGE)) {
				weekThing.loadGraphic(Paths.image('storyMenu/weekImages/' + assetName));
				isMissing = false;
			}
		}
		...
		if(isMissing) {
			...
			missingFileText.text = 'MISSING FILE: images/storyMenu/weekImages/' + assetName + '.png';
		}
