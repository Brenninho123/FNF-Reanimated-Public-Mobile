...
class EditorPlayState extends MusicBeatSubstate
{
			...
			for (note in unspawnNotes) {
				for (type in 0...Note.useDefaultNoteTexture.length) {
					if (note.noteType == Note.useDefaultNoteTexture[type]) {
						note.notefolder = 'torchs_assets';
						note.texture = PlayState.isPixelStage ? 'custom_notes/pixelUI/normal' : 'custom_notes/normal';
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...4)
		{
			...
			if (ClientPrefs.data.noteSkin == 'Character Based') {
					babyArrow = new StrumNote(strumLineX, strumLineY, i, player, PlayState.isPixelStage ? 'custom_notes/pixelUI/normal' : 'custom_notes/normal', 'torchs_assets');
