...
class PhillyOG extends BaseStage
{
    ...
    override function create()
        {
            if(!ClientPrefs.data.lowQuality) {
                var bg:BGSprite = new BGSprite('philly/OG/sky', -810, -500, 0.1, 0.1);
                add(bg);
                
                var city:BGSprite = new BGSprite('philly/OG/city', -760, -280, 0.3, 0.3);
            ...
            var edificio:BGSprite = new BGSprite('philly/OG/edificio', -760, -280, 0.3, 0.3);
            edificio.setGraphicSize(Std.int(edificio.width * 0.85));
            edificio.updateHitbox();
            add(edificio);
    
            var ventanas:BGSprite = new BGSprite('philly/OG/ventanas', -760, -280, 0.3, 0.3);
            ...
            phillyWindow = new BGSprite('philly/OG/window', ventanas.x, ventanas.y, 0.3, 0.3);
            ...
            if(!ClientPrefs.data.lowQuality) {
                phillyPmg = new BGSprite('philly/OG/pmg', -1130, 540);
            }
    
            phillyTrain = new PhillyTrain(2000, -160);
            add(phillyTrain);
    
            phillyStreet = new BGSprite('philly/OG/street', -1370, 120);
            add(phillyStreet);

            switch(PlayState.SONG.song.toLowerCase()) {
                case 'blammed' | 'verbal smash':
        ...
        override function eventPushed(event:objects.Note.EventNote)
        {
            switch(event.event)
            {
                case "Philly Glow":
                    ...
                    phillyWindowEvent = new BGSprite('philly/OG/window', phillyWindow.x, phillyWindow.y, 0.3, 0.3);
                    phillyWindowEvent.setGraphicSize(Std.int(phillyWindowEvent.width * 0.85));
                    phillyWindowEvent.updateHitbox();
                    phillyWindowEvent.visible = false;
                    insert(members.indexOf(blammedLightsBlack) + 1, phillyWindowEvent);
    
                    phillyGlowGradient = new PhillyGlowGradient(-400, 275); //This shit was refusing to properly load FlxGradient so fuck it
