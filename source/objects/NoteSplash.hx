...
class NoteSplash extends FlxSprite
{
	...
	public function new(x:Float = 0, y:Float = 0, ?library:String = null) {
		super(x, y);
	...
	var maxAnims:Int = 2;
	public function setupNoteSplash(x:Float, y:Float, direction:Int = 0, ?note:Note = null, ?library:String = null, ?noteType:String = '') {
		...
		if (note.texture.contains('pixelUI/')) {
			shader = null; // Making null first in case there are issues
			rgbShader = new PixelSplashShaderRef(true);
			shader = rgbShader.shader;
		}
		
		var config:NoteSplashConfig = null;
		if(_textureLoaded != texture)
			config = loadAnims(texture, null, library, note);
		else
			config = precacheConfig(_configLoaded, library);
	...
	function altSkin(?assets:String = '', ?note:Note = null):String {
		var tableOfSkins:Array<String> = [
			'custom_notes/notesplashes/parents'
		];

		if (note != null && note.noteType == 'Alt Animation') {
	...
	function loadAnims(skin:String, ?animName:String = null, ?library:String = null, ?note:Note = null):NoteSplashConfig {
		maxAnims = 0;
		...
		config = precacheConfig(skin, library);
		_configLoaded = skin;
		...
		while(true) {
			...
			for (i in 0...Note.colArray.length) {
				//if (!addAnimAndCheck('note$i-$animID', '$animName ${Note.colArray[i]} $animID' + '0', 24, false)) {return config;}

				if (!addAnimAndCheck('note$i-$animID', '$animName ${Note.colArray[i]} $animID' + altSkin(skin, note), 24, false)) {return config;}
			}
			maxAnims++;
	...
	public static function precacheConfig(skin:String, ?library:String = null)
	{
		if(configs.exists(skin)) return configs.get(skin);

		var path:String = Paths.getPath('images/$skin.txt', TEXT, library, true);
...
class PixelSplashShaderRef {
	...
	public function new(?pixelNote:Bool = false)
	{
		...
		var pixel:Float = 1;
		if(PlayState.isPixelStage || pixelNote) pixel = PlayState.daPixelZoom;
